...
public class StudentServiceImpl implements StudentService{
	...
	public SqlSession getSqlSession() {// 기존 getConnection 과 같음
		// getSqlSession 메소드의 역할은
		// DB에 연결할 JDBC 연결 코드를 통하여 연결하는 세션 정보를
		// 리턴해주는 역할
		
		// Mybatis에서 SqlSession 을 만들기 위한 순서 (SqlSession == Connection 객체)
		// 1. 드라이버 및 JDBC 연결 정보를 가지고 있는 xml 파일을 읽어 들여야 함( InputStream 생성)
		// 2. SqlSession 을 만들려면 SqlSessionFactory 객체가 필요함
		...
		try {
			...
			// false 값은 auto commit 을 해제(기본값은 true)
	...
	public int deleteUser(int studentNo) {
		SqlSession session = getSqlSession();
		int result = new StudentDAOImpl().deleteuser(session, studentNo);
		if(result>0)
		{
			session.commit();
		}
